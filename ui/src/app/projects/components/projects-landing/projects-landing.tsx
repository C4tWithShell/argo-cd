                    ...
                    load={async () => {
                        const [projects, apps, appSets] = await Promise.all([
                            ...
                            services.applications.list([], 'application', {
                                fields: ['items.metadata.name', 'items.spec.project', 'items.metadata.ownerReferences', 'items.status.health']
                            }),
                            services.applications.list([], 'applicationset', {fields: ['items.metadata.name', 'items.metadata.namespace', 'items.status.health']})
                        ]);

                        // Count apps per project and appsets per project
                        const appCountByProject: {[key: string]: number} = {};
                        const appSetNames = new Set<string>();
                        const appSetCountByProject: {[key: string]: number} = {};

                        // Collect appset names for cross-referencing
                        (appSets.items || []).forEach((appSet: models.AbstractApplication) => {
                            const name = appSet.metadata.name || '';
                            appSetNames.add(name);
                            // AppSets have template.spec.project in the raw spec
                            const project: string = appSet.spec?.template?.spec?.project || 'default';
